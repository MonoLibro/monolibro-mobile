...
class ActivityHistory extends StatefulWidget{
  const ActivityHistory({Key? key, required this.text, required this.index}) : super(key: key);
  final Map text;
  final int index;
...
class _ActivityHistoryState extends State<ActivityHistory>{
  ...
  Widget buildListItem(BuildContext context, int index, int maxIndex, bool pending, String date, double price, String title){
    double position = 0;
    double size = 60;
    if (maxIndex == 1){
      size = 0;
    ...
    Color textColor = pending ? ThemeColors.defaultAccent[3] : ThemeColors.successAccent[1];
    Color dotColor = pending ? ThemeColors.defaultAccent[3] : ThemeColors.successAccent[0];
    return Material(
          ...
          child: SizedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  for (var j = 0; j < 20; j ++)
                  buildListItem(context, j, 20, j == 0, "2020-10-10", 40, "Title_$j"),
                ]
              ),
            )
          )
